...
import YhteystiedotContext from "./AthletesContext";
...
const GlobalState = (props) => {
  ...
  const getYhteystiedot = async () => {
    try {
      let res = await axios.get("http://localhost:3000/athlete");
      let { data } = res;
  ...
  const getYhteystieto = async (id) => {
    try {
      let sql = "http://localhost:3000/athlete/" + id;
      //alert(sql);
  ...
  const setYhteystiedot = async (uusiPuhelintieto) => {
    try {
      const res = await axios
        .post(`http://localhost:3000/lisaa`, uusiPuhelintieto)
        .then((res) => {
  ...
  const setYhteystieto = async (id, paivitettyPuhelintieto) => {
    try {
      const res = await axios
        .put(`http://localhost:3000/athlete/${id}`, paivitettyPuhelintieto)
        .then((res) => {
  ...
  const poistaYhteystieto = async (id) => {
    try {
      let sql = "http://localhost:3000/athlete/" + id["id"];
